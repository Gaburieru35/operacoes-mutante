...
describe('Suíte de Testes Fraca para 50 Operações Aritméticas', () => {
  ...
  test('6. deve calcular a raiz quadrada de um quadrado perfeito', () => { expect(raizQuadrada(16)).toBe(4);
    expect(() => raizQuadrada(-2)).toThrow('Não é possível calcular a raiz quadrada de um número negativo.');
    expect(() => raizQuadrada(16)).not.toThrow('Não é possível calcular a raiz quadrada de um número negativo.');
    expect(() => raizQuadrada(0)).not.toThrow('Não é possível calcular a raiz quadrada de um número negativo.');
   });
  ...
  test('8. deve calcular o fatorial de um número maior que 1', () => { expect(fatorial(4)).toBe(24);
    ...
    //expect(() => fatorial(false)).toThrow('Fatorial não é definido para números negativos.');
    expect(fatorial(0)).toBe(1);
    expect(fatorial(1)).toBe(1);
    expect(fatorial(2)).toBe(2);
    expect(fatorial(3)).toBe(6);
    expect(fatorial({ valueOf: () => 5 })).toBe(120);
  });
  ...
  test('33. deve verificar que um número é primo', () => { expect(isPrimo(7)).toBe(true);
    ...
    expect(isPrimo(false)).toBe(false);
  ...
  test('35. deve calcular o produto de um array', () => { expect(produtoArray([2, 3, 4])).toBe(24);
     ...
     expect(() => produtoArray(false)).toThrow();
     expect(() => produtoArray(null)).toThrow();
   });
  test('36. deve manter um valor dentro de um intervalo (clamp)', () => { expect(clamp(5, 0, 10)).toBe(5);
    ...
    const min = { valueOf: () => 10 };
    const max = { valueOf: () => 15 };
    expect(clamp(valor, min, max)).toBe(valor);

    const valorMax = { valueOf: () => 15 };
    const min2 = { valueOf: () => 10 };
    const max2 = { valueOf: () => 15 };
    expect(clamp(valorMax, min2, max2)).toBe(valorMax);

    const low = { valueOf: () => 0 };
    const high = { valueOf: () => 20 };
    const min3 = { valueOf: () => 10 };
    const max3 = { valueOf: () => 15 };
    expect(clamp(low, min3, max3)).toBe(min3);
    expect(clamp(high, min3, max3)).toBe(max3);
  ...
  test('37. deve verificar se um número é divisível por outro', () => { expect(isDivisivel(10, 2)).toBe(true);
    ...
    //expect(() => isDivisivel(10, 0)).toThrow();
    expect(isDivisivel(-10, 2)).toBe(true);
    expect(isDivisivel(10, -3)).toBe(false);
   });
  ...
  test('47. deve calcular a mediana de um array ímpar e ordenado', () => { expect(medianaArray([1, 2, 3, 4, 5])).toBe(3);
    expect(medianaArray([7, 1, 3, 5, 9])).toBe(5);
    expect(medianaArray([1, 2, 3, 4])).toBe(2.5);
    expect(medianaArray([8, 3, 1, 4])).toBe(3.5);
